...
export function useTransactions(
  ...
  return useQuery<TransactionsResponse>({
    ...
    queryFn: async () => {
      ...
      const response = await backendApi.get(`/api/billing/transactions?${params.toString()}`);
      if (response.error) {
...
export function useTransactionsSummary(days: number = 30) {
  return useQuery<TransactionsSummary>({
    ...
    queryFn: async () => {
      const response = await backendApi.get(`/api/billing/transactions/summary?days=${days}`);
      if (response.error) {
        throw new Error(response.error.message);
      }
      return response.data;
